...
//   const { data, error } = await api.GET("/holdings");
//   if (error) throw error;
//   return data ?? [];
// }

// export async function addHolding(payload: {
//   symbol: string; shares: number; avgCost: number; currency?: "USD"|"CAD";
// }) {
//   const { data, error } = await api.POST("/holdings", { body: payload });
//   if (error) throw error;
//   return data!;
// }
...
export function Dashboard() {
  ...
  const handleAddStock = (newStock: Omit<Stock, 'id' | 'marketValue' | 'totalReturn' | 'totalReturnPercent'>) => {
    ...
    const updatedStocks = [...portfolioData.stocks, stock];
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="holdings" className="space-y-6">
        ...
        <TabsContent value="holdings" className="space-y-6">
          <HoldingsTable stocks={portfolioData.stocks} onRemoveStock={handleRemoveStock} />
        </TabsContent>
